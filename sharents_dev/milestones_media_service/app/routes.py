...
from .utils import check_delete_result, check_update_result, check_for_none, check_list_not_empty
from .schemas import MediaCollection, MediaModel, MediaModelCreate, MediaModelUpdate, MilestoneCollection, MilestoneModel, CommentCollection, CommentModel, CommentModelCreate, MilestoneModelCreate, MilestoneModelUpdate
...
async def post_media_for_child(child_id: str, media: MediaModelCreate):
    ...
    inserted_media = await db.get_collection("media").find_one({"child": child_id, "_id": result.inserted_id})
...
async def post_milestone_for_child(child_id: str, milestone: MilestoneModelCreate):
    ...
    inserted_milestone = await db.get_collection("milestones").find_one({"child": child_id, "_id": result.inserted_id})
...
async def update_media(media_id: str, media: MediaModelUpdate):
    media_dict = media.model_dump()
    result = await db.get_collection("media").update_one({"_id": ObjectId(media_id)}, {"$set": media_dict})
    check_update_result(result, "media not found")
    updated_media = await db.get_collection("media").find_one({"_id": ObjectId(media_id)})
...
async def update_milestone(milestone_id: str, milestone: MilestoneModelUpdate):
    milestone_dict = milestone.model_dump()
    result = await db.get_collection("milestones").update_one({"_id": ObjectId(milestone_id)}, {"$set": milestone_dict})
    check_update_result(result, "milestone not found")
    updated_milestone = await db.get_collection("milestones").find_one({"_id": ObjectId(milestone_id)})
...
async def delete_milestone(milestone_id: str):
    result = await db.get_collection("milestones").delete_one({"_id": ObjectId(milestone_id)})
